...
import { state, streakState, accessibilityState, companionData, multiplayerState, questState, adaptiveStats, totalSessionsPlayed, perfectSessions, sessionHistory } from './state.js';
import { CATEGORIES, BADGES, FOOTER_PHRASES, COMPANIONS, COMPANION_STAGES } from './constants.js';
import { updateStarsUI, updateBadgeCountUI, showToast, updateFooterMessage, updateDifficultyUI, updateMultiplayerScoreboardUI, updateQuestObjectiveUI, updateFooterCompanion, getCompanion, renderRecommendations } from './ui.js';
import { playAudioTone } from './audio.js';
import { getGameStats, saveAdaptiveStats } from './storage.js';
...
function badgeSvg(shape, bgColor, borderColor, icon, iconColor, isLocked) {
  ...
  const bg = shape === 'shield'
    ? `<path d="M50 5 L88 27 V55 C88 78 50 95 50 95 C50 95 12 78 12 55 V27 L50 5 Z" fill="${bgColor}" stroke="${borderColor}" stroke-width="4"/><path d="M50 12 L82 30 V52 C82 72 50 86 50 86 C50 86 18 72 18 52 V30 L50 12 Z" fill="white" stroke="${borderColor}" stroke-width="1.5" opacity="0.85"/>`
    : `<circle cx="50" cy="50" r="45" fill="${bgColor}" stroke="${borderColor}" stroke-width="4"/><circle cx="50" cy="50" r="39" fill="white" stroke="${borderColor}" stroke-width="1.5" opacity="0.85"/>`;
  return `<svg class="w-full h-full ${lock}" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">${bg}<text x="50" y="58" text-anchor="middle" font-size="36" fill="${iconColor}" font-family="serif">${icon}</text></svg>`;
}

function getBadgeReqText(badge) {
  const c = badge.cond;
  switch (c.type) {
    case 'stars': return `${c.value} 🌟`;
    case 'streak': return `Série ${c.value}j`;
    case 'accuracy': return `${Math.round(c.value * 100)}%`;
    case 'speed': return `<${c.value / 1000}s`;
    case 'game_total': return `${c.value} rep.`;
    case 'game_total_any': return `${c.value} rep.`;
    case 'game_correct': return `${c.value} ✓`;
    case 'category_correct': return `${c.value} ✓`;
    case 'difficulty': return c.value === 'normal' ? 'Niv. Normal' : 'Niv. Défi';
    case 'challenge_mastered': return `${c.value} ✓ Défi`;
    case 'sessions': return `${c.value} sessions`;
    case 'perfect_session': return `${c.value} parfaites`;
    case 'all_games_played': return 'Tous les jeux';
    case 'session_stars': return `${c.value} ⭐/session`;
    case 'consecutive': return `${c.value} d'affilée`;
    case 'time_of_day': return c.value === 20 ? 'Après 20h' : 'Avant 8h';
    default: return '';
  }
}
...
export function isBadgeConditionMet(cond) {
  switch (cond.type) {
    case 'stars': return state.stars >= cond.value;
    case 'streak': return streakState.current >= cond.value;
    case 'accuracy': {
      let totalCorrect = 0, totalAttempts = 0;
      for (const stats of Object.values(adaptiveStats)) { totalCorrect += stats.correct; totalAttempts += stats.total; }
      if (totalAttempts < 20) return false;
      return (totalCorrect / totalAttempts) >= cond.value;
    }
    case 'speed': {
      let allTimes = [];
      for (const stats of Object.values(adaptiveStats)) allTimes = allTimes.concat(stats.recentTimes);
      if (allTimes.length < 10) return false;
      return (allTimes.reduce((a, b) => a + b, 0) / allTimes.length) <= cond.value;
    }
    case 'game_total': { const stats = adaptiveStats[cond.game]; return stats && stats.total >= cond.value; }
    case 'game_total_any': { for (const stats of Object.values(adaptiveStats)) { if (stats.total >= cond.value) return true; } return false; }
    case 'game_correct': { const stats = adaptiveStats[cond.game]; return stats && stats.correct >= cond.value; }
    case 'category_correct': {
      const cat = CATEGORIES[cond.category];
      if (!cat) return false;
      let total = 0;
      for (const g of cat.games) { const s = adaptiveStats[g.id]; if (s) total += s.correct; }
      return total >= cond.value;
    }
    case 'difficulty': return state.difficulty === cond.value;
    case 'challenge_mastered': {
      let totalChallenge = 0;
      for (const stats of Object.values(adaptiveStats)) { if (stats.difficulty === 'challenge') totalChallenge += stats.correct; }
      return totalChallenge >= cond.value;
    }
    case 'sessions': return totalSessionsPlayed >= cond.value;
    case 'perfect_session': return perfectSessions >= cond.value;
    case 'all_games_played': {
      const allGameIds = [];
      for (const cat of Object.values(CATEGORIES)) for (const g of cat.games) allGameIds.push(g.id);
      return allGameIds.every(id => { const s = adaptiveStats[id]; return s && s.total > 0; });
    }
    case 'session_stars': return state.sessionStars >= cond.value;
    case 'consecutive': return state.consecutiveCorrect >= cond.value;
    case 'time_of_day': { const hour = new Date().getHours(); return cond.value === 20 ? hour >= 20 : hour < 8; }
...
export function checkBadgeUnlocks() {
  ...
  BADGES.forEach(badge => {
    if (!state.unlockedBadges.includes(badge.id) && isBadgeConditionMet(badge.cond)) {
      state.unlockedBadges.push(badge.id);
      newlyUnlocked.push(badge);
    }
  });
  if (newlyUnlocked.length > 0) {
    localStorage.setItem('mathscp_unlocked_badges', JSON.stringify(state.unlockedBadges));
    updateBadgeCountUI();
...
function triggerBadgeUnlockPopup(badge) {
  ...
  if (descEl) descEl.innerText = `Félicitations ! Tu as débloqué le badge "${badge.name}" ! ${badge.desc} 🏆`;
...
export function getCompanionStage() {
  ...
  for (let i = COMPANION_STAGES.length - 1; i >= 0; i--) {
    if (state.stars >= COMPANION_STAGES[i].minStars) { stage = i; break; }
  }
  return stage;
}
...
export function launchQuest(questId) {
  ...
  if (questId === 1) {
    questState.materialEmoji = "🪵"; questState.materialName = "Brindilles";
    questState.gameSequence = ['a_complements_10', 'a_nombres_100', 'a_blocs_dizaines', 'a_additions', 'a_fractions_interact'];
  } else if (questId === 2) {
    questState.materialEmoji = "🔑"; questState.materialName = "Clés";
    questState.gameSequence = ['b_vocabulaire_spatial', 'b_figures', 'b_formes_tri', 'b_vocabulaire_spatial', 'b_formes_tri'];
  } else {
    questState.materialEmoji = "⚙️"; questState.materialName = "Engrenages";
    questState.gameSequence = ['c_heure', 'c_mesures', 'c_monnaie', 'c_mesures', 'c_monnaie'];
  }
...
function showQuestStoryIntro() {
  ...
  if (questState.currentQuest === 1) {
    avatar.innerText = "🦉🪺"; title.innerText = "Le Nid de Pytha";
    text.innerHTML = "<b>Pytha l'Hibou</b> : <br/>\"Salut jeune mathématicien ! L'hiver approche et je dois réparer mon nid douillet. Peux-tu m'aider à ramasser <b>5 brindilles</b> en résolvant mes énigmes de nombres ? C'est parti !\"";
  } else if (questState.currentQuest === 2) {
    avatar.innerText = "🦉🔑"; title.innerText = "Le Temple Perdu";
    text.innerHTML = "<b>Pytha l'Hibou</b> : <br/>\"Regarde ces ruines mystérieuses ! La porte du temple est fermée. Trouve les <b>5 clés de pierre</b> cachées derrière les formes géométriques pour l'ouvrir ! En route !\"";
  } else {
    avatar.innerText = "🦉⚙️"; title.innerText = "Le Trésor du Temps";
    text.innerHTML = "<b>Pytha l'Hibou</b> : <br/>\"Oh non ! L'horloge magique s'est arrêtée, le temps est figé ! Récolte <b>5 engrenages dorés</b> en lisant l'heure et en mesurant des objets pour la relancer ! Vite !\"";
  }
...
export function nextQuestStep() {
  if (questState.consecutiveCorrect >= 2) {
    questState.currentSequenceIndex = Math.min(questState.gameSequence.length - 1, questState.currentSequenceIndex + 2);
    questState.consecutiveCorrect = 0;
    showToast(true, "Super rapide ! Saut de niveau ! 🚀");
  } else {
    questState.currentSequenceIndex = (questState.currentSequenceIndex + 1) % questState.gameSequence.length;
  }
  state.currentGame = questState.gameSequence[questState.currentSequenceIndex];
  document.getElementById('game-title').innerText = `Quête : ${getGameName(state.currentGame)}`;
  generateExercise();
}

export function triggerQuestVictory() {
  playAudioTone('trophy');
  const badgeNames = ["", "Protecteur du Nid 🪺", "Clé du Savoir 🔑", "Maître du Temps ⏳"];
...
export function startMultiplayerMatch() {
  ...
  document.getElementById('star-dots-container').parentNode.classList.add('hidden');
...
export function triggerMultiplayerMatchOver() {
  ...
  hideAllScreens();
  document.getElementById('screen-multiplayer-victory').classList.remove('hidden');
  confetti({ particleCount: 150, spread: 80, origin: { y: 0.6 } });
}
...
function saveMatchResult(p1Name, p1Stars, p2Name, p2Stars, winnerName) {
  let history = JSON.parse(localStorage.getItem('mathscp_multiplayer_history') || '[]');
  history.unshift({ date: new Date().toLocaleDateString('fr-FR'), p1: { name: p1Name, stars: p1Stars }, p2: { name: p2Name, stars: p2Stars }, winner: winnerName });
...
export function openScoreboardModal() {
  ...
  if (!container) return;
  container.innerHTML = '';
  const history = JSON.parse(localStorage.getItem('mathscp_multiplayer_history') || '[]');
  if (history.length === 0) {
    ...
    history.forEach(match => {
      const card = document.createElement('div');
      card.className = "bg-slate-50 p-4 rounded-2xl border-2 border-slate-200/60 flex flex-col gap-2 font-title";
      const winnerLabel = match.winner === "Égalité"
...
export function updateAccessibilityUI() {
  ...
  if (accessibilityState.autoplayVoice) {
    voiceBtn.className = "w-12 h-6 rounded-full bg-brand-green p-0.5 transition-all relative";
    voiceBtn.querySelector('div').className = "w-5 h-5 rounded-full bg-white shadow-md transform transition-all translate-x-6";
  } else {
    voiceBtn.className = "w-12 h-6 rounded-full bg-slate-200 p-0.5 transition-all relative";
    voiceBtn.querySelector('div').className = "w-5 h-5 rounded-full bg-white shadow-md transform transition-all translate-x-0";
  }
  if (accessibilityState.highContrast) {
    contrastBtn.className = "w-12 h-6 rounded-full bg-brand-green p-0.5 transition-all relative";
    contrastBtn.querySelector('div').className = "w-5 h-5 rounded-full bg-white shadow-md transform transition-all translate-x-6";
  } else {
    contrastBtn.className = "w-12 h-6 rounded-full bg-slate-200 p-0.5 transition-all relative";
    contrastBtn.querySelector('div').className = "w-5 h-5 rounded-full bg-white shadow-md transform transition-all translate-x-0";
  }
  document.querySelectorAll('#question-count-options button').forEach(btn => {
    const count = parseInt(btn.dataset.count);
    if (count === state.questionCount) {
      btn.className = "flex-1 py-2 px-3 rounded-xl text-sm font-bold border-2 bg-brand-purple text-white border-brand-purple shadow transition-all";
    } else {
      btn.className = "flex-1 py-2 px-3 rounded-xl text-sm font-bold border-2 border-slate-200 text-slate-500 bg-white hover:border-brand-purple-light hover:text-brand-purple transition-all";
    }
  });
...
export function toggleAccVoice() {
  ...
  localStorage.setItem('mathscp_autoplay_voice', accessibilityState.autoplayVoice.toString());
  updateAccessibilityUI();
}
...
export function toggleAccContrast() {
  ...
  localStorage.setItem('mathscp_high_contrast', accessibilityState.highContrast.toString());
  updateAccessibilityUI();
  applyAccessibilitySettings();
}
...
export function updateStreak() {
  ...
  if (streakState.lastPlayDate !== today) {
    ...
    if (streakState.current > streakState.best) streakState.best = streakState.current;
    localStorage.setItem('mathscp_streak_current', streakState.current.toString());
...
export function getStreakText() {
  ...
  if (streakState.current <= 2) return `${streakState.current} jour${streakState.current > 1 ? 's' : ''} 🔥`;
...
export function checkStreakBonus() {
  if (streakState.current >= 3 && streakState.current % 3 === 0) {
    ...
    localStorage.setItem('mathscp_stars', state.stars);
    updateStarsUI();
    setTimeout(() => showToast(true, `Série de ${streakState.current} jours ! ${bonusStars} étoiles bonus ! 🌟🔥`), 600);
  }
}
...
let shownFacts = JSON.parse(localStorage.getItem('mathscp_shown_facts') || '[]');
...
export function showRandomFact() {
  ...
  if (available.length === 0) { shownFacts = []; }
...
import { playtimeState, parentGateAnswer, parentGateMode } from './state.js';
...
export function initParentalControls() {
  ...
  if (playtimeState.lockoutStart === 0 && playtimeState.lastActive > 0 && (now - playtimeState.lastActive) >= DECAY_BREAK_DURATION) {
...
function tickPlaytime() {
  ...
  localStorage.setItem('mathscp_playtime_accumulated', playtimeState.accumulated.toString());
  localStorage.setItem('mathscp_last_active_time', playtimeState.lastActive.toString());
  if (playtimeState.accumulated >= PLAYTIME_LIMIT) {
    playtimeState.lockoutStart = now;
    localStorage.setItem('mathscp_lockout_start_time', playtimeState.lockoutStart.toString());
    triggerLockout();
  }
}
...
function updateCountdownUI() {
  ...
  const remaining = LOCKOUT_DURATION - (Date.now() - playtimeState.lockoutStart);
  if (remaining <= 0) { textEl.innerText = "00:00"; return; }
  const totalSecs = Math.floor(remaining / 1000);
  textEl.innerText = `${Math.floor(totalSecs / 60).toString().padStart(2, '0')}:${(totalSecs % 60).toString().padStart(2, '0')}`;
}
...
export function setGameMode(mode) {
  ...
  btnSolo.className = "px-4 py-2 rounded-xl font-title font-bold text-sm transition-all text-slate-600";
  ...
  if (mode === 'multi') {
    ...
    btnMulti.className = "px-4 py-2 rounded-xl font-title font-bold text-sm transition-all shadow bg-white text-brand-purple";
  } else if (mode === 'quest') {
    questState.isActive = true;
    btnQuest.className = "px-4 py-2 rounded-xl font-title font-bold text-sm transition-all shadow bg-white text-brand-purple";
    setTimeout(() => { hideAllScreens(); document.getElementById('screen-quests').classList.remove('hidden'); }, 200);
  } else {
