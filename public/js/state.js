...
export let state = {
  ...
  unlockedBadges: JSON.parse(
    localStorage.getItem('mathscp_unlocked_badges') || '[]'
  ),
  consecutiveCorrect: 0,
  consecutiveWrong: 0,
  difficulty: 'normal',
  questionCount: parseInt(
    localStorage.getItem('mathscp_question_count') || '10'
  ),
  questionNumber: 0,
  secondChancesLeft: 0,
};
...
export let shownFacts = JSON.parse(
...
export let adaptiveStats = JSON.parse(
  localStorage.getItem('mathscp_adaptive_stats') || '{}'
);
...
export let totalSessionsPlayed = parseInt(
  localStorage.getItem('mathscp_total_sessions') || '0'
);
export let perfectSessions = parseInt(
  localStorage.getItem('mathscp_perfect_sessions') || '0'
);
export let totalTimePlayed = parseInt(
  localStorage.getItem('mathscp_playtime_accumulated') || '0'
);

export let sessionHistory = JSON.parse(
...
export let playtimeState = {
  accumulated: parseInt(
    localStorage.getItem('mathscp_playtime_accumulated') || '0'
  ),
  lastActive: parseInt(localStorage.getItem('mathscp_last_active_time') || '0'),
  lockoutStart: parseInt(
    localStorage.getItem('mathscp_lockout_start_time') || '0'
  ),
...
export let companionData = JSON.parse(
  localStorage.getItem('mathscp_companion') || '{"type":"owl"}'
);
