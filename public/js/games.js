...
import { state, multiplayerState, questState, userFractionSelected, targetFractionNumerator, targetFractionDenominator, userTens, userUnits, targetNumber, targetShapeType, shapesArray } from './state.js';
import { getRandomInt, shuffleArray, getDistractors, markSeen, isRecentlySeen, clearRecentlySeen, getGameName } from './utils.js';
import { playAudioTone, speakText } from './audio.js';
import { updateMultiplayerScoreboardUI, updateQuestObjectiveUI, updateStarsUI, updateSessionStarsUI, updateDifficultyUI, showToast, updateFooterMessage, renderOptions, updateProgressBar, hideProgressBar } from './ui.js';
import { recordAnswer, getGameStats, getAdaptiveThreshold, recordSession } from './storage.js';
...
import { questionStartTime, questionTimerInterval, questionTimeRemaining } from './state.js';
...
export function startQuestionTimer() {
  stopQuestionTimer();
  stateMod.questionTimeRemaining = QUESTION_TIME_LIMIT;
  updateTimerUI();
  const timerEl = document.getElementById('question-timer');
  if (timerEl) timerEl.classList.remove('timer-urgent');
  stateMod.questionTimerInterval = setInterval(() => {
    stateMod.questionTimeRemaining--;
    updateTimerUI();
    if (stateMod.questionTimeRemaining <= 5 && stateMod.questionTimeRemaining > 0) {
      const timerEl = document.getElementById('question-timer');
      if (timerEl) timerEl.classList.add('timer-urgent');
      const timerText = document.getElementById('timer-text');
      if (timerText) timerText.classList.add('timer-shake-anim');
      setTimeout(() => { if (timerText) timerText.classList.remove('timer-shake-anim'); }, 300);
    }
...
export function stopQuestionTimer() {
  if (stateMod.questionTimerInterval) {
    ...
    stateMod.questionTimerInterval = null;
  }
}
...
export function checkAnswer(selected, buttonElement) {
  ...
  if (isCorrect) {
    ...
    buttonElement.classList.add('border-brand-green', 'bg-brand-green-light', 'text-brand-green-dark');
    ...
    if (!multiplayerState.isActive) {
      const elapsed = stateMod.questionStartTime ? Date.now() - stateMod.questionStartTime : null;
    ...
    if (!multiplayerState.isActive && !questState.isActive) {
      ...
      if (state.difficulty === 'easy' && state.consecutiveCorrect >= upThreshold) {
        state.difficulty = 'normal';
        state.consecutiveCorrect = 0;
        setTimeout(() => showToast(true, "Super ! Difficulté : Normal ! 🔵"), 500);
        updateDifficultyUI();
      } else if (state.difficulty === 'normal' && state.consecutiveCorrect >= upThreshold) {
        state.difficulty = 'challenge';
        state.consecutiveCorrect = 0;
        setTimeout(() => showToast(true, "Incroyable ! Niveau Défi débloqué ! 🔥"), 500);
        updateDifficultyUI();
      }
    }
    ...
    if (!multiplayerState.isActive && !questState.isActive && state.sessionStars < state.questionCount) {
      showRandomFact();
    }
    if (multiplayerState.isActive) {
      ...
      if (nextBtn) {
        if (multiplayerState.currentTurn >= multiplayerState.maxTurns) {
          ...
          const nextPlayer = multiplayerState.currentPlayer === 1 ? multiplayerState.player2 : multiplayerState.player1;
          nextBtn.querySelector('span').innerText = `Passer au tour de ${nextPlayer.name} ${multiplayerState.currentPlayer === 1 ? "🦁" : "🐣"}`;
        }
      ...
      if (state.sessionStars >= state.questionCount) {
        checkStreakBonus();
        setTimeout(() => { triggerVictorySession(); }, 1000);
      } else {
    ...
    playAudioTone('fail');
    if (!multiplayerState.isActive && !questState.isActive && state.secondChancesLeft > 0) {
      state.secondChancesLeft--;
      state.isAnswerSelected = false;
      buttonElement.classList.remove('border-slate-200');
      buttonElement.classList.add('border-brand-pink', 'bg-brand-pink-light', 'text-brand-pink-dark', 'opacity-50', 'cursor-not-allowed');
      buttonElement.onclick = null;
      setTimeout(() => showToast(false, "Hmm, essaie encore ! 💪"), 300);
      return;
    }
    buttonElement.classList.remove('border-slate-200');
    buttonElement.classList.add('border-brand-pink', 'bg-brand-pink-light', 'text-brand-pink-dark', 'shake');
    if (multiplayerState.isActive) {
      const nextBtn = document.getElementById('btn-next-exercise');
      if (nextBtn) {
        if (multiplayerState.currentTurn >= multiplayerState.maxTurns) {
      ...
      if (state.difficulty === 'challenge' && state.consecutiveWrong >= downThreshold) {
        state.difficulty = 'normal';
        state.consecutiveWrong = 0;
        setTimeout(() => showToast(false, "Retour au niveau Normal 🔵"), 500);
        updateDifficultyUI();
      } else if (state.difficulty === 'normal' && state.consecutiveWrong >= downThreshold) {
        state.difficulty = 'easy';
        state.consecutiveWrong = 0;
        setTimeout(() => showToast(false, "Mode Facile activé pour t'aider ! 🟢"), 500);
        updateDifficultyUI();
      }
      checkBadgeUnlocks();
    }
  ...
  if (optionsContainer) {
    optionsContainer.querySelectorAll('button').forEach(btn => {
      if (btn.innerText == state.currentAnswer) {
        ...
        btn.classList.add('border-brand-green', 'bg-brand-green-light', 'text-brand-green-dark');
      }
    });
  }
  if (multiplayerState.isActive) {
    ...
    const elapsed = stateMod.questionStartTime ? QUESTION_TIME_LIMIT * 1000 : null;
    recordAnswer(state.currentGame, false, elapsed);
    state.consecutiveWrong++;
    state.consecutiveCorrect = 0;
    const downThreshold = getAdaptiveThreshold(state.currentGame);
...
export function generateExercise() {
  ...
  stateMod.questionStartTime = Date.now();
  ...
  for (let attempt = 0; attempt < MAX_DEDUP_ATTEMPTS; attempt++) {
    ...
    switch (state.currentGame) {
      case 'a_nombres_100': genNombres100(visualArea, questionText, optionsContainer); break;
      case 'a_additions': genAdditions(visualArea, questionText, optionsContainer); break;
      case 'a_soustractions': genSoustractions(visualArea, questionText, optionsContainer); break;
      case 'a_doubles_moities': genDoublesMoities(visualArea, questionText, optionsContainer); break;
      case 'a_complements_10': genComplements10(visualArea, questionText, optionsContainer); break;
      case 'a_fractions': genFractions(visualArea, questionText, optionsContainer); break;
      case 'b_figures': genFigures(visualArea, questionText, optionsContainer); break;
      case 'b_vocabulaire_spatial': genVocabulaireSpatial(visualArea, questionText, optionsContainer); break;
      case 'b_formes_tri': genFormesTri(visualArea, questionText, optionsContainer); break;
      case 'c_monnaie': genMonnaie(visualArea, questionText, optionsContainer); break;
      case 'c_heure': genHeure(visualArea, questionText, optionsContainer); break;
      case 'c_mesures': genMesures(visualArea, questionText, optionsContainer); break;
      case 'a_fractions_interact': genFractionsInteract(visualArea, questionText, optionsContainer); break;
      case 'a_blocs_dizaines': genBlocsDizaines(visualArea, questionText, optionsContainer); break;
    }
  ...
  setTimeout(() => {
    if (!state.isMuted || (state.accessibilityState && state.accessibilityState.autoplayVoice)) {
...
function genNombres100(visualArea, questionText, optionsContainer) {
  ...
  if (state.difficulty === 'easy') {
    ...
    const pairs = [[12,21],[13,31],[14,41],[15,51],[16,61],[17,71],[18,81],[19,91],[23,32],[24,42],[25,52],[26,62],[27,72],[28,82],[29,92],[34,43],[35,53],[36,63],[37,73],[38,83],[39,93],[45,54],[46,64],[47,74],[48,84],[49,94],[56,65],[57,75],[58,85],[59,95],[67,76],[68,86],[69,96],[78,87],[79,97],[89,98]];
  ...
  if (mode === 1) {
    ...
    const dWidth = 12, dHeight = 120, spacing = 18, startX = 20, startY = 30;
    ...
    const cSize = 12, uStartX = startX + (dizaines * spacing) + 20, uStartY = startY + 10;
    ...
    renderOptions(number, getDistractors(number, 3, state.difficulty === 'easy' ? 1 : 10, state.difficulty === 'easy' ? 25 : 99), optionsContainer);
  } else {
...
function genAdditions(visualArea, questionText, optionsContainer) {
  ...
  if (mode === 1) {
    let a, b, correct;
    if (state.difficulty === 'easy') { a = getRandomInt(1, 7); b = getRandomInt(1, 10 - a); }
    else {
      const sm = getRandomInt(1, 3);
      if (sm === 1) { a = getRandomInt(2, 9); b = getRandomInt(2, 9); }
      else if (sm === 2) { a = getRandomInt(1, 8) * 10; b = getRandomInt(1, 9); }
      else { a = getRandomInt(11, 25); b = getRandomInt(1, 4); }
    }
    correct = a + b;
    ...
    renderOptions(correct, getDistractors(correct, 3, 2, 100), optionsContainer);
  } else {
    let aUnits, bUnits, aTens, bTens;
    if (state.difficulty === 'challenge') {
      aUnits = getRandomInt(4, 9); bUnits = getRandomInt(10 - aUnits, 9);
      aTens = getRandomInt(1, 4); bTens = getRandomInt(1, 4);
    } else {
      aUnits = getRandomInt(1, 6); bUnits = getRandomInt(1, 9 - aUnits);
      aTens = getRandomInt(1, 5); bTens = getRandomInt(1, 9 - aTens);
    }
    const a = aTens * 10 + aUnits, b = bTens * 10 + bUnits, correct = a + b;
    state.currentAnswer = correct;
    ...
    renderOptions(correct, getDistractors(correct, 3, 10, 99), optionsContainer);
  }
}
...
function genSoustractions(visualArea, questionText, optionsContainer) {
  let a, b;
  if (state.difficulty === 'easy') { a = getRandomInt(3, 9); b = getRandomInt(1, a - 1); }
  else if (state.difficulty === 'challenge') {
    a = getRandomInt(15, 29); b = getRandomInt(11, a - 2);
    if (a % 10 < b % 10) { a = Math.floor(a / 10) * 10 + getRandomInt(5, 9); b = Math.floor(b / 10) * 10 + getRandomInt(1, 4); }
  } else {
    const aTens = getRandomInt(1, 2), aUnits = getRandomInt(4, 9), bTens = aTens === 2 ? getRandomInt(1, 2) : 0, bUnits = getRandomInt(1, aUnits - 1);
    a = aTens * 10 + aUnits; b = bTens * 10 + bUnits;
  }
  const correct = a - b;
  state.currentAnswer = correct;
  ...
  let svg = `<svg class="w-full h-full max-h-[300px]" viewBox="0 0 320 200" xmlns="http://www.w3.org/2000/svg">`;
  const r = 12, spacingX = 30, spacingY = 32;
  for (let i = 0; i < a; i++) {
    const col = i % 10, row = Math.floor(i / 10), x = 25 + col * spacingX, y = 35 + row * spacingY;
  ...
  svg += `</svg>`;
  visualArea.innerHTML = svg;
  renderOptions(correct, getDistractors(correct, 3, 0, state.difficulty === 'easy' ? 10 : 35), optionsContainer);
}
  ...
  if (mode === 1) {
    ...
    const drawStarsInBox = (sx, count) => {
      ...
      for (let i = 0; i < count; i++) {
        const col = i % 3, row = Math.floor(i / 3), px = sx + 25 + col * 35, py = 50 + row * 40;
    ...
    svg += `</svg>`;
    visualArea.innerHTML = svg;
    renderOptions(correct, getDistractors(correct, 3, 2, state.difficulty === 'easy' ? 12 : 35), optionsContainer);
  } else {
    let Y;
    ...
    const drawCandies = (sx, count) => {
      let cs = '';
      for (let i = 0; i < count; i++) {
        const col = i % 3, row = Math.floor(i / 3), px = sx + 22 + col * 35, py = 50 + row * 40;
...
function genComplements10(visualArea, questionText, optionsContainer) {
  let x, correct;
  if (state.difficulty === 'easy') { x = getRandomInt(4, 9); correct = 10 - x; }
  else if (state.difficulty === 'challenge') { x = getRandomInt(11, 18); correct = 20 - x; }
  else { x = getRandomInt(1, 9); correct = 10 - x; }
  state.currentAnswer = correct;
  ...
  if (state.difficulty === 'challenge') {
    ...
    for (let i = 0; i < 20; i++) {
      const grid = Math.floor(i / 10), cellInGrid = i % 10, row = Math.floor(cellInGrid / 5), col = cellInGrid % 5;
      const cx = 35 + col * 50 + 25, cy = (grid === 0 ? 20 : 105) + row * 35 + 17.5;
      if (i < x) {
    ...
    let svg = `<svg class="w-full h-full max-h-[300px]" viewBox="0 0 320 200" xmlns="http://www.w3.org/2000/svg">`;
    const gridW = 250, gridH = 100, startX = 35, startY = 50, cellW = gridW / 5, cellH = gridH / 2;
    svg += `<rect x="${startX}" y="${startY}" width="${gridW}" height="${gridH}" rx="10" fill="#F8FAFC" stroke="#6C5DD3" stroke-width="4"/>`;
    for (let i = 1; i < 5; i++) svg += `<line x1="${startX + i * cellW}" y1="${startY}" x2="${startX + i * cellW}" y2="${startY + gridH}" stroke="#6C5DD3" stroke-width="2"/>`;
    svg += `<line x1="${startX}" y1="${startY + cellH}" x2="${startX + gridW}" y2="${startY + cellH}" stroke="#6C5DD3" stroke-width="2"/>`;
    for (let i = 0; i < 10; i++) {
      const row = Math.floor(i / 5), col = i % 5, cx = startX + col * cellW + cellW / 2, cy = startY + row * cellH + cellH / 2;
      if (i < x) {
...
function genFigures(visualArea, questionText, optionsContainer) {
  if (state.difficulty === 'challenge') {
    ...
    while (distractorsSet.size < 3) { const d = getRandomInt(0, 6); if (d !== correct) distractorsSet.add(d); }
    renderOptions(`${correct}`, Array.from(distractorsSet).map(v => `${v}`), optionsContainer);
  } else {
    const shapes = [
      { id: 'carré', name: 'un carré', color: '#6C5DD3', border: '#5646B8' },
      { id: 'rectangle', name: 'un rectangle', color: '#4D96FF', border: '#357AE8' },
      { id: 'triangle', name: 'un triangle', color: '#4ECA64', border: '#3AB24F' },
    ...
    let shapeMarkup = '', faceX = 120, faceY = 120;
    if (chosen.id === 'carré') { shapeMarkup = `<rect x="50" y="50" width="140" height="140" rx="12" fill="${chosen.color}" stroke="${chosen.border}" stroke-width="6"/>`; }
    else if (chosen.id === 'rectangle') { shapeMarkup = `<rect x="30" y="65" width="180" height="110" rx="12" fill="${chosen.color}" stroke="${chosen.border}" stroke-width="6"/>`; faceY = 120; }
    else if (chosen.id === 'triangle') { shapeMarkup = `<polygon points="120,40 35,190 205,190" stroke-linejoin="round" fill="${chosen.color}" stroke="${chosen.border}" stroke-width="6"/>`; faceY = 145; }
    else if (chosen.id === 'cercle') { shapeMarkup = `<circle cx="120" cy="120" r="75" fill="${chosen.color}" stroke="${chosen.border}" stroke-width="6"/>`; }
    svg += shapeMarkup;
    ...
    svg += `</svg>`;
    visualArea.innerHTML = svg;
    renderOptions(chosen.name, shapes.map(s => s.name).filter(n => n !== chosen.name), optionsContainer);
  }
}
...
function genVocabulaireSpatial(visualArea, questionText, optionsContainer) {
  let positions = [
    ...
    { id: 'en dessous', label: 'en dessous de la boîte', birdX: 120, birdY: 195 },
    { id: 'à gauche', label: 'à gauche de la boîte', birdX: 35, birdY: 130 },
    { id: 'à droite', label: 'à droite de la boîte', birdX: 205, birdY: 130 }
  ];
  if (state.difficulty === 'easy') {
    positions = getRandomInt(0, 1) === 0 ? [positions[0], positions[1]] : [positions[2], positions[3]];
  }
  ...
  const bx = chosen.birdX, by = chosen.birdY;
  ...
  svg += `</svg>`;
  visualArea.innerHTML = svg;
  renderOptions(chosen.label, positions.map(p => p.label).filter(l => l !== chosen.label), optionsContainer);
}
...
function genMonnaie(visualArea, questionText, optionsContainer) {
  ...
  if (state.difficulty === 'easy') {
    for (let i = 0; i < getRandomInt(0, 2); i++) { items.push({ type: 'piece', val: 2 }); total += 2; }
    for (let i = 0; i < getRandomInt(1, 3); i++) { items.push({ type: 'piece', val: 1 }); total += 1; }
  } else if (state.difficulty === 'challenge') {
    for (let i = 0; i < getRandomInt(1, 3); i++) { items.push({ type: 'billet', val: 10 }); total += 10; }
    for (let i = 0; i < getRandomInt(1, 2); i++) { items.push({ type: 'billet', val: 5 }); total += 5; }
    for (let i = 0; i < getRandomInt(1, 3); i++) { items.push({ type: 'piece', val: 2 }); total += 2; }
    for (let i = 0; i < getRandomInt(1, 3); i++) { items.push({ type: 'piece', val: 1 }); total += 1; }
  } else {
    for (let i = 0; i < getRandomInt(0, 2); i++) { items.push({ type: 'billet', val: 10 }); total += 10; }
    for (let i = 0; i < getRandomInt(0, 2); i++) { items.push({ type: 'billet', val: 5 }); total += 5; }
    for (let i = 0; i < getRandomInt(0, 3); i++) { items.push({ type: 'piece', val: 2 }); total += 2; }
    for (let i = 0; i < getRandomInt(1, 3); i++) { items.push({ type: 'piece', val: 1 }); total += 1; }
  }
  if (total < 1) { items.push({ type: 'piece', val: 2 }); total += 2; }
  state.currentAnswer = `${total} €`;
  state.currentQuestionKey = 'mon_' + state.difficulty + '_' + items.map(i => i.val).sort().join(',');
  ...
  items.forEach((item, index) => {
    const row = Math.floor(index / 3), col = index % 3, x = 70 + col * 65 + (row * 15), y = 80 + row * 38;
    if (item.type === 'billet') {
  ...
  svg += `</svg>`;
  visualArea.innerHTML = svg;
  renderOptions(`${total} €`, getDistractors(total, 3, 1, 60).map(v => `${v} €`), optionsContainer);
}
...
function genHeure(visualArea, questionText, optionsContainer) {
  function getHeureText(h, min) {
    ...
    if (min === 45) { const nh = h === 12 ? 1 : h + 1; return `${nh} heures moins le quart`; }
    return `${h} heures`;
  }
  let hour = getRandomInt(1, 12), min = 0;
  if (state.difficulty === 'easy') min = 0;
  else if (state.difficulty === 'challenge') min = [0, 15, 30, 45][getRandomInt(0, 3)];
  ...
  for (let h = 1; h <= 12; h++) { const angle = h * 30 * Math.PI / 180; svg += `<text x="${cx + (r - 20) * Math.sin(angle)}" y="${cy - (r - 20) * Math.cos(angle) + 5}" font-family="Fredoka" font-weight="bold" font-size="14" fill="#334155" text-anchor="middle">${h}</text>`; }
  for (let m = 0; m < 60; m += 5) { if (m % 15 !== 0) { const angle = m * 6 * Math.PI / 180; svg += `<line x1="${cx + (r - 8) * Math.sin(angle)}" y1="${cy - (r - 8) * Math.cos(angle)}" x2="${cx + (r - 4) * Math.sin(angle)}" y2="${cy - (r - 4) * Math.cos(angle)}" stroke="#CBD5E1" stroke-width="2"/>`; } }
  const mAngle = min * 6 * Math.PI / 180; const mx = cx + 68 * Math.sin(mAngle), my = cy - 68 * Math.cos(mAngle);
  svg += `<line x1="${cx}" y1="${cy}" x2="${mx}" y2="${my}" stroke="#4D96FF" stroke-width="5" stroke-linecap="round"/>`;
  const hAngle = (hour + min / 60) * 30 * Math.PI / 180; const hx = cx + 45 * Math.sin(hAngle), hy = cy - 45 * Math.cos(hAngle);
  ...
  while (choicesSet.size < 4) {
    const dh = getRandomInt(1, 12); let dmin = 0;
    if (state.difficulty === 'easy') dmin = 0;
    else if (state.difficulty === 'challenge') dmin = [0, 15, 30, 45][getRandomInt(0, 3)];
    else dmin = [0, 30][getRandomInt(0, 1)];
    choicesSet.add(getHeureText(dh, dmin));
  }
  renderOptions(correctText, Array.from(choicesSet).filter(t => t !== correctText), optionsContainer);
}

// ---- C3. Mesures ----
function genMesures(visualArea, questionText, optionsContainer) {
  let length = state.difficulty === 'easy' ? getRandomInt(3, 7) : getRandomInt(3, 11);
  ...
  let svg = `<svg class="w-full h-full max-h-[300px]" viewBox="0 0 320 200" xmlns="http://www.w3.org/2000/svg">`;
  const rulerX = 20, rulerY = 110, rulerW = 280, rulerH = 40;
  ...
  for (let i = 0; i <= 12; i++) {
    ...
    if (i < 12) { const hx = x + pxPerCm / 2; svg += `<line x1="${hx}" y1="${rulerY}" x2="${hx}" y2="${rulerY + 9}" stroke="#CA8A04" stroke-width="1.5"/>`; }
  }
  const crayonY = 60, crayonH = 20, crayonW = length * pxPerCm;
  ...
  const tipStartX = rulerX + crayonW - 15, tipMidY = crayonY + crayonH / 2;
  ...
  visualArea.innerHTML = svg;
  renderOptions(`${length} cm`, getDistractors(length, 3, 2, 12).map(v => `${v} cm`), optionsContainer);
}
...
function genFractions(visualArea, questionText, optionsContainer) {
  ...
  let svg = `<svg class="w-full h-full max-h-[300px]" viewBox="0 0 320 200" xmlns="http://www.w3.org/2000/svg">`;
  const width = 240, height = 60, startX = 40, startY = 70, blockW = width / d;
  for (let i = 0; i < d; i++) {
    const bx = startX + i * blockW, isColored = i < n;
  ...
  renderOptions(`${n}/${d}`, allFractions.filter(f => f !== `${n}/${d}`).slice(0, 3), optionsContainer);
}
...
function genFractionsInteract(visualArea, questionText, optionsContainer) {
  stateMod.targetFractionDenominator = [2, 3, 4][getRandomInt(0, 2)];
  stateMod.targetFractionNumerator = getRandomInt(1, stateMod.targetFractionDenominator - 1);
  state.currentAnswer = `${stateMod.targetFractionNumerator}/${stateMod.targetFractionDenominator}`;
  state.currentQuestionKey = 'fint_' + stateMod.targetFractionNumerator + '_' + stateMod.targetFractionDenominator;
  stateMod.userFractionSelected = Array(stateMod.targetFractionDenominator).fill(false);
...
function renderFractionsInteractSVG(visualArea) {
  ...
  const width = 240, height = 60, startX = 40, startY = 70, blockW = width / stateMod.targetFractionDenominator;
  for (let i = 0; i < stateMod.targetFractionDenominator; i++) {
    const bx = startX + i * blockW, isColored = stateMod.userFractionSelected[i];
...
function renderBlocsDizainesSVG(visualArea) {
  ...
  for (let t = 0; t < stateMod.userTens; t++) {
    ...
    for (let b = 0; b < 10; b++) svg += `<rect x="${sx}" y="${160 - b * 13}" width="16" height="11" fill="#CBBFEF" stroke="#6C5DD3" stroke-width="1.5" rx="1"/>`;
  }
  const unitStartX = 180;
  for (let u = 0; u < stateMod.userUnits; u++) {
    const col = u % 4, row = Math.floor(u / 4), ux = unitStartX + col * 28, uy = 150 - row * 28;
...
export function addBaseTen(value) {
  playAudioTone('click');
  if (value === 10) { if (stateMod.userTens < 5) stateMod.userTens++; }
  else { if (stateMod.userUnits < 15) stateMod.userUnits++; }
...
export function validateBaseTen() {
  ...
  const valBtn = document.querySelector('#game-options-container button:last-child');
...
function genFormesTri(visualArea, questionText, optionsContainer) {
  ...
  stateMod.targetShapeType = types[getRandomInt(0, 2)];
  const shapeNamesFr = { triangle: "triangles 🔺", rectangle: "rectangles 🟦", cercle: "cercles 🟡" };
  questionText.innerText = `Trouve et clique sur tous les ${shapeNamesFr[stateMod.targetShapeType]} dans le dessin :`;
  stateMod.shapesArray = [];
  for (let i = 0; i < 6; i++) {
    let type = types[getRandomInt(0, 2)];
    if (i < 2) type = stateMod.targetShapeType;
    stateMod.shapesArray.push({ id: i, type: type, selected: false, x: 0, y: 0 });
  }
  stateMod.shapesArray = shuffleArray(stateMod.shapesArray);
  stateMod.shapesArray.forEach((shape, idx) => { shape.x = 55 + (idx % 3) * 95; shape.y = 55 + Math.floor(idx / 3) * 75; });
  state.currentAnswer = "correct";
  state.currentQuestionKey = 'form_' + stateMod.targetShapeType + '_' + stateMod.shapesArray.map(s => s.type).join(',');
...
function renderFormesTriSVG(visualArea) {
  ...
  stateMod.shapesArray.forEach(shape => {
    ...
    if (shape.type === 'triangle') svg += `<polygon points="${shape.x + 30},${shape.y} ${shape.x},${shape.y + 50} ${shape.x + 60},${shape.y + 50}" fill="${fill}" stroke="${stroke}" stroke-width="${strokeWidth}"/>`;
    else if (shape.type === 'rectangle') svg += `<rect x="${shape.x}" y="${shape.y + 5}" width="60" height="40" fill="${fill}" stroke="${stroke}" stroke-width="${strokeWidth}" rx="4"/>`;
    else if (shape.type === 'cercle') svg += `<circle cx="${shape.x + 30}" cy="${shape.y + 25}" r="22" fill="${fill}" stroke="${stroke}" stroke-width="${strokeWidth}"/>`;
    svg += `</g>`;
  });
  svg += `</svg>`;
  visualArea.innerHTML = svg;
...
export function validateFormesTri() {
  const allCorrectSelected = stateMod.shapesArray.every(shape => {
    return shape.type === stateMod.targetShapeType ? shape.selected : !shape.selected;
  });
